 ...
 * @property {() => import("../primitives/ui.js").Control} render - Main body factory
 */
 ...
 * @property {import("./NavigationView.js").NavigationViewOptions} [layout] - NavigationView options
 */

/**
 * App chrome built on {@link NavigationView}: sidebar nav + main content host.
 *
 * @example <caption>Compact shell</caption>
 * // live
 ...
 * @category Shell
 */
class AppShell extends NavigationView {
  /**
  ...
  constructor(options = {}) {
    ...
    /** @type {SpaNavGroup[]} */
    this.groups = [];
    /** @type {Map<string, InputList>} */
    this.groupLists = new Map();
    /** @type {Map<string, CollapsiblePanel>} */
    this.groupSections = new Map();
    /** @type {TabView | null} */
    this.parentTabs = null;
    /** @type {string|null} */
    this.currentPageId = null;
    /** @type {Map<string, import("../primitives/ui.js").Control>} */
    this.pageCache = new Map();
    this.onNavigate = onNavigate;

    this.mainTitle = new TextBlock("");
    this.mainTitle.setClass("Title");

    this.mainSubtitle = new Caption("");
    this.mainSubtitle.setStyle("display", ["block"]);
    this.mainSubtitle.setStyle("marginTop", ["0.25rem"]);

    const headerText = new StackPanel({ isVertical: true });
    headerText.gap("0");
    ...
    this.mainBody.addClass("AppShell-body");

    this.navHost = new StackPanel({ isVertical: true });
    this.navHost.gap("0.25rem");
    this.navHost.addClass("AppShell-nav");
   ...
   * @param {CollapsiblePanel} section
  ...
  mountLeafGroup(leaf, section, { parentGroupId } = {}) {
    ...
    (leaf.items ?? []).forEach((item) => {
      ...
      const listItem = new InputListItem(list);
  ...
  buildLeafColumn(group, { parentGroupId, expandFirst = false } = {}) {
    const host = new StackPanel({ isVertical: true });
    host.gap("0.25rem");
    host.addClass("AppShell-nestedGroups");
    ...
    leaves.forEach((child) => {
      if (child.type === "divider") {
        const rule = new Line();
        rule.addClass("AppShell-navDivider");
        host.add(rule);
        return;
      }

      if (child.type === "heading") {
        const heading = new TextBlock(child.label ?? "");
        heading.addClass("AppShell-navHeading");
        host.add(heading);
        return;
      }

      const childSection = new CollapsiblePanel({
        title: child.label,
  ...
  setGroups(groups) {
    ...
    if (allNested) {
      this.navHost.addClass("AppShell-nav--tabs");
      const tabs = new TabView();
      tabs.addClass("AppShell-groupTabs");
    ...
    this.navHost.removeClass("AppShell-nav--tabs");

    groups.forEach((group, index) => {
      const section = new CollapsiblePanel({
        title: group.label,
