import { Rectangle, InputDate, Container, InputText } from "../../primitives/ui.js";

let agGridModulePromise;
let agGridModulesRegistered = false;

/** @category Peers */
function loadAgGrid() {
  agGridModulePromise ??= import("ag-grid-community");
  return agGridModulePromise;
}

/** @category Peers */
function ensureAgGridModulesRegistered(agGrid) {
    ...
    agGrid.provideGlobalGridOptions({ theme: "legacy" });
...
/** @category Peers */
class DataGrid extends Rectangle {
    constructor(options = {}) {
        super();
    ...
    getDateTimeCellEditor() {
        ...
        DateTimeCellEditor.prototype.init = function (params) {
            ...
            this.container = new Container().setStyles({
                width: "100%",
                height: "100%",
            }).dom;
            ...
            if (this.value) {
                ...
                if (!Number.isNaN(parsedDate.getTime())) {
                    this.input.setValue(parsedDate.toLocaleString());
                }
            }

            this.container.appendChild(this.input.dom);

            this.datePicker = new InputDate(this.value);
            ...
            this.datePicker.dom.addEventListener("change", () => {
                ...
                this.input.setValue(selectedDate.toLocaleString());
                this.value = selectedDate.toISOString();
            });
        };

        DateTimeCellEditor.prototype.showDatePicker = function () {
            const rect = this.input.dom.getBoundingClientRect();
    ...
    initializeAgGrid() {
        void loadAgGrid()
            .then((agGrid) => {
                ensureAgGridModulesRegistered(agGrid);

                this.gridApi = agGrid.createGrid(this.dom, this.gridOptions);
    ...
    getSelected() {
        ...
        const selectedNodeGraph = this.gridApi.getSelectedNodeGraph();

        if (selectedNodeGraph.length === 0) {
            return null;
        }

        return {
            nodes: selectedNodeGraph,
            data: selectedNodeGraph.map((node) => node.data),
        };
    }
