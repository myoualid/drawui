...
class TreeView {
   ...
   * @param {Function} [options.renderItem] - (item, tree, depth) => Control
  ...
  refresh() {
    ...
    const roots = this._roots();
    if (roots.length === 0) {
      const empty = new TextBlock(this.emptyMessage);
  ...
  defaultRenderItem(item, tree, depth) {
    ...
    if (hasChildren) {
      const icon = new Icon(expanded ? "expand_more" : "chevron_right");
