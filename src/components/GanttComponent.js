...
import { SimpleFloatingWindow } from "../layout/BasePanel.js";
...
class GanttComponent {
    ...
    showDependencyDialog(task) {
        ...
        const content = new UIPanel().addClass("ws-dependency-dialog-content");
        content.add(new UIText(`Manage Dependencies for "${taskData.Name}"`).addClass("Title"));

        const currentSection = new UIPanel().addClass("ws-dependency-section");
        currentSection.add(new UIText("Current Dependencies").addClass("Title"));

        const dependencies = taskData.Dependencies || [];
        const depTypes = taskData.DepType || [];

        if (dependencies.length === 0) {
            const noDependencies = new UIText("No dependencies");
            noDependencies.dom.style.color = "#999";
            noDependencies.dom.style.fontStyle = "italic";
            currentSection.add(noDependencies);
        } else {
            const dependencyList = new UIListbox().addClass("ws-dependency-items");

            for (let index = 0; index < dependencies.length; index++) {
                const dependencyItem = new UIPanel();
                ...
                dependencyItem.add(
                    new UIText(`${predecessorName} (ID: ${dependencies[index]}) - ${this.getDependencyTypeLabel(depTypes[index])}`)
                );

                const removeButton = new UIButton("Remove");
        ...
        addSection.add(new UIText("Add New Dependency").addClass("Title"));

        const addForm = new UIPanel();
        const predecessorLabel = new UIText("Predecessor Task:");
        const predecessorSelect = new UISelect();
        ...
        const typeLabel = new UIText("Type:");
        const typeSelect = new UISelect();
        ...
        const addButton = new UIButton("Add Dependency");
        ...
        const panel = new SimpleFloatingWindow({ context: this.context });
        panel
            .setTitle(`Manage Dependencies for "${taskData.Name}"`)
            .setContent(content)
            .show();
