import { UIDiv, UIColumn, UIIcon, UISpan, UIText } from "../primitives/ui.js";
...
export class PieMenu {
	...
	draw() {
		...
		const dragHandle = new UIDiv();

		dragHandle.addClass("PieMenu-dragHandle");

		const dragIcon = new UIIcon("drag_indicator");
		...
		const hubLabel = new UIText(this.getHubLabel());
	...
	buildItems(metrics) {
		...
		for (let index = 0; index < this.items.length; index++) {
			...
			slot.addClass("PieMenu-itemSlot").addClass(`PieMenu-itemSlot-${index}`);
			slot.setStyles({
				left: `${x}px`,
				top: `${y}px`,
