...
import { makeResizable, makeDraggable } from "./drag.js";
 ...
 * @category Overlays
 */
class FloatingWindow extends Rectangle {
  static get isMobile() {
  ...
  constructor(options) {
    ...
    /**
     * When set, dock-from-float restores body into this WorkspacePanel instead of a WorkspaceDockHost wrapper.
     * @type {object|null}
     */
    this._sourceWorkspacePanel = options?.sourceWorkspacePanel || null;
  ...
  * Attach to the layout floating layer (typically #World) and sync geometry.
   ...
   */
  prepareAfterRemount() {
    if (FloatingWindow.isMobile) return this;
  ...
  _buildHeader() {
    const header = new Container()
      .addClass("Row")
      .addClass("FloatingWindow-header")
      .addClass("Header");

    this.add(header);

    const titleRow = new StackPanel({ isVertical: false }).setStyles({
      alignItems: "center",
      flex: "1 1 auto",
      gap: "0.5rem",
      minWidth: "0",
    ...
    const tools = new StackPanel({ isVertical: false }).setStyles({
      alignItems: "center",
      flexShrink: "0",
      gap: "0.25rem",
      justifyContent: "flex-end",
    });
    tools.addClass("FloatingWindow-tools");
  ...
  _buildContentWrapper() {
    this.contentWrapper = new Container().addClass("fill-height").addClass("FloatingWindow-content");

    this.add(this.contentWrapper);
  }

  _createHeaderButton(iconName, onClick, tooltip = "") {
    const button = new Icon(iconName);
    button.addClass("Operator");
    button.addClass("FloatingWindow-tool");
  ...
  _applyPosition(config) {
    if (FloatingWindow.isMobile) return;
  ...
  _buildContent(options) {
    ...
    if (options.input) {
      const input = new InputText(options.input.defaultValue || "");
    ...
    if (options.panel) {
      ...
      for (const tab of options.panel.tabs) {
        const tabContent = new Rectangle();

        for (const element of tab.content) {
          tabContent.add(element);
        }

        tabView.addTab(tab.title, tabContent);
      }

      this.contentWrapper.add(tabView);
    }
    ...
    if (options.confirm || options.cancel) {
      ...
      buttonContainer.dom.style.gap = "var(--dui-space-3)";

      buttonContainer.dom.style.justifyContent = "flex-end";

      if (options.confirm) {
        const confirmButton = new Button(options.confirm.text);
      ...
      if (options.cancel) {
        const cancelButton = new Button(options.cancel.text);
